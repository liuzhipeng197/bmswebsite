...
<script language="javascript">
	...
	function changetestobj(){
		...
		/*$.post("./task.php",{action:"staff_conquery",str_condition:str_condition,str_class:str_class},function(data){
				if(data){
					//alert(data);
	...
	function condition_query(){
		...
		if(obj_type[0].checked){
			...
			str_class=obj.selectedIndex+1;
			if(str_condition==''&&str_class==1){
				alert("请输入查询条件");
				return;	
			}else if(str_condition==''&&str_class==2){
				alert("请输入查询条件");
				return;
			}else if(str_class==3){
				return;
	...
	function del_info(str_id,str_cusname,str_deal){//根据id号来删除配置环境的信息
		if(str_deal=="接受"){
			alert("不允许修改已接受的任务信息！");
			return;
		}
		var str_logname="<?php echo $_SESSION['staff_realname']?>";			//获取日志管理操作人id
		//alert(str_realname);
		$.post("./task.php",{action:"cus_del",str_logname:str_logname,str_id:str_id,str_cusname:str_cusname},function(data){
		  	if(data){
		  		alert('删除成功');
		  		window.location.href="./custom_show.php";
		  	}
		});
	}
</script>
