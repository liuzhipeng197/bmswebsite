...
<script language="javascript">
	window.onload=init;
	function init(){
		$.post("./task.php",{action:"manage_init"},function(data){
		  if(data){
		  	//alert(data);
	...
	function del_manage(userid){//根据id号来删除配置环境的信息
		...
		//{
			$.post("./task.php",{action:"manage_del",userId:userid},function(data){
		  		if(data){
		  			alert('删除成功');
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  		<tr>
  			...
  			<td width="100%" align="left" ><div id="right_topcenter">当前位置：管理员管理 > 查看管理员</div></td>
