...
<script language="javascript">
	window.onload=init;
	function init(){
		$.post("./edit_task.php",{action:"download_init"},function(data){
		  if(data){
		  	//alert(data);
	...
	function showdownload_bytype(){
		var type=$('#type').val();
		if(type==''){
			//alert('请选择正确的动态信息类型！');
		}else{
			//alert(type);
			$.post("./edit_task.php",{action:"showdownload_bytype",type:type},function(data){
		  if(data){
		  	//alert(data);
			$('#display_res').html(data);
		  }
	...
	function del_coreinfo(coreinfoid){//根据id号来删除配置环境的信息
		...
		//{
			$.post("./edit_task.php",{action:"download_del",coreinfoId:coreinfoid},function(data){
		  		if(data){
		  			alert('删除成功');
		  			window.location.href="./download_show.php";
		  		}
			});
		//}
	}
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
  		<tr>
  			...
  			<td width="100%" align="left" ><div id="right_topcenter">当前位置：下载中心 > 查看下载</div></td>
...
<tr><!-- 标题栏-1 -->
	<td height="30" background="../../../images/tab_05.gif">
        ...
        <tr>
            ...
            <td><img src="../../../images/tb.gif" width="16" height="16" />&nbsp;&nbsp;<b>下载列表</b>
            <select id="type" name="type" onchange="showdownload_bytype()" style="text-algin:center">
