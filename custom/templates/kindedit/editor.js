...
var BOTTOM_TOOLBAR_ICON = Array(
						...
						Array('Ft_FLV', 'flv.gif', '插入FLV'),
						Array('Ft_MEDIA', 'media.gif', '插入MediaPlayer'),
						Array('Ft_REAL', 'real.gif', '插入RealPlayer'),
...
var POPUP_MENU_TABLE = Array("Ft_ZOOM", "Ft_TITLE", "Ft_FONTNAME", "Ft_FONTSIZE", "Ft_TEXTCOLOR", "Ft_BGCOLOR", 
							"Ft_LAYER", "Ft_TABLE", "Ft_HR", "Ft_ICON", "Ft_SPECIALCHAR", "Ft_IMAGE", "Ft_FLASH", "Ft_FLV", "Ft_MEDIA", "Ft_REAL", "Ft_LINK");
...
function FtConvertAttribute(str)
{
	...
	str = FtConvertAttributeChild(str, 'face', '[^\"\'>]+');
...
function FtDisplayMenu(cmd)
{
    ...
    if (cmd == 'Ft_LINK') {
		left -= 220;
	}else if (cmd == 'Ft_IMAGE' || cmd == 'Ft_FLASH' || cmd == 'Ft_FLV' || cmd == 'Ft_MEDIA' || cmd == 'Ft_REAL') {
	    left -= 230;
	}else if (cmd == 'Ft_ICON'){
	    left -= 145;
	}
...
function FtDrawMedia(cmd)
{
	...
	str += '<table cellpadding="0" cellspacing="0" style="width:100%">' + 
		'<tr><td colspan="2" align="left" style="padding:5px;">'+STR_IMAGE_REMOTE+' <input type="text" id="'+cmd+'link" value="http://" style="width:190px;border:1px solid #CAD9EA;" /></td></tr>' +
...
function FtPopupMenu(cmd)
{
	switch (cmd)
	{
		...
		case 'Ft_MEDIA':
			var str = '';
			str += '<div id="POPUP_'+cmd+'" style="width:250px;'+FtGetMenuCommonStyle()+'">';
			str += '<iframe name="FtMediaIframe" id="FtMediaIframe" frameborder="0" style="width:250px;height:95px;padding:0;margin:0;border:0;">';
			str += '</iframe></div>';
			return str;
			break;
		case 'Ft_REAL':
			var str = '';
			str += '<div id="POPUP_'+cmd+'" style="width:250px;'+FtGetMenuCommonStyle()+'">';
			str += '<iframe name="FtRealIframe" id="FtRealIframe" frameborder="0" style="width:250px;height:95px;padding:0;margin:0;border:0;">';
			str += '</iframe></div>';
			return str;
			break;
		case 'Ft_LINK':
...
function FtDrawIframe(cmd)
{
	if (BROWSER == 'IE') {
		...
		FLV_DOCUMENT = document.frames("FtFlvIframe").document;
		MEDIA_DOCUMENT = document.frames("FtMediaIframe").document;
		REAL_DOCUMENT = document.frames("FtRealIframe").document;
		...
		FLV_DOCUMENT = document.getElementById('FtFlvIframe').contentDocument;
		MEDIA_DOCUMENT = document.getElementById('FtMediaIframe').contentDocument;
		REAL_DOCUMENT = document.getElementById('FtRealIframe').contentDocument;
	...
	switch (cmd)
	{
		...
		case 'Ft_FLV':
			var str = '';
			str += '<table cellpadding="0" cellspacing="0" style="width:100%;">' + 
			'<tr><td style="padding:8px;">地址 <input type="text" id="flvLink" value="http://" style="width:195px;border:1px solid #CAD9EA;" /></td></tr>' +
			'<tr><td style="padding-left:8px;padding-bottom:8px;">宽度 <input name="w" type="text" id="w" size="3" value="500" style="border:1px solid #CAD9EA;" /> 高度 <input name="h" type="text" id="h" value="400" size="3" style="border:1px solid #CAD9EA;"/> ' +
			'<select name="auto" id="auto"><option value="0" selected="selected">手动</option><option value="1">自动</option></td></tr>' +
			'<tr><td colspan="2" align="center">' +
			'<input type="submit" name="button" id="'+cmd+'submitButton" value="'+STR_BUTTON_CONFIRM+'" onclick="javascript:parent.FtDrawFlvEnd();" style="border:1px solid #CAD9EA;background-color:'+BUTTON_COLOR+';" /> ' +
			'<input type="button" name="button" value="'+STR_BUTTON_CANCEL+'" onclick="javascript:parent.FtDisableMenu();" style="border:1px solid #CAD9EA;background-color:'+BUTTON_COLOR+';" /></td></tr>' + 
			'</table>';
			FtWriteFullHtml(FLV_DOCUMENT, str);
			FLV_DOCUMENT.body.style.color = MENU_TEXT_COLOR;
			FLV_DOCUMENT.body.style.backgroundColor = MENU_BG_COLOR;
			FLV_DOCUMENT.body.style.margin = 0;
			FLV_DOCUMENT.body.scroll = 'no';
			break;
		case 'Ft_MEDIA':
			var str = '';
			str += FtDrawMedia(cmd);
		...
		case 'Ft_REAL':
			var str = '';
			str += FtDrawMedia(cmd);
			FtWriteFullHtml(REAL_DOCUMENT, str);
...
function FtDrawFlvEnd()
{
	var url = FLV_DOCUMENT.getElementById('flvLink').value;
	var w = FLV_DOCUMENT.getElementById('w').value;
	var h = FLV_DOCUMENT.getElementById('h').value;
	var auto = FLV_DOCUMENT.getElementById('auto').value;
	if (auto==0){
	autocode='false';
	}else if (auto==1){
	autocode='true';
	}
	if (FtCheckFlvFileType(url, "/") == false) {
		return false;
	}
	if (w.match(/^\d+$/) == null) {
		alert(MSG_INVALID_WIDTH);
...
function FtDrawMediaEnd(cmd)
{
	...
	if (FtCheckMediaFileType(cmd, url, "/") == false) {
		return false;
	}
	if (w.match(/^\d+$/) == null) {
		alert(MSG_INVALID_WIDTH);
...
function FtExecute(cmd, value)
{
	...
	{
		case 'Ft_SOURCE':
			var length = document.getElementById(TOP_TOOLBAR_ICON[18][0]).src.length - 10;
			var image = document.getElementById(TOP_TOOLBAR_ICON[18][0]).src.substr(length,10);
			if (image == 'source.gif') {
		...
		case 'Ft_FLV':
			FtDisplayMenu(cmd);
			FtFlvIframe.focus();
			FLV_DOCUMENT.getElementById(cmd+'submitButton').focus();
			break;
		case 'Ft_MEDIA':
			FtDisplayMenu(cmd);
			FtMediaIframe.focus();
...
{
	if (flag == true) {
		document.getElementById(TOP_TOOLBAR_ICON[18][0]).src = IMAGE_PATH + 'design.gif';
		for (i = 0; i < TOOLBAR_ICON.length; i++) {
			var el = document.getElementById(TOOLBAR_ICON[i][0]);
			if (TOOLBAR_ICON[i][0] == 'Ft_DATE' || TOOLBAR_ICON[i][0] == 'Ft_TIME'||TOOLBAR_ICON[i][0] == 'Ft_HR' || TOOLBAR_ICON[i][0] == 'Ft_LINK'|| TOOLBAR_ICON[i][0] == 'Ft_TABLE'|| TOOLBAR_ICON[i][0] == 'Ft_UNLINK'|| TOOLBAR_ICON[i][0] == 'Ft_IMAGE'|| TOOLBAR_ICON[i][0] == 'Ft_FLASH'|| TOOLBAR_ICON[i][0] == 'Ft_FLV'|| TOOLBAR_ICON[i][0] == 'Ft_MEDIA'|| TOOLBAR_ICON[i][0] == 'Ft_REAL'|| TOOLBAR_ICON[i][0] == 'Ft_ICON') {
		...
		document.getElementById(TOP_TOOLBAR_ICON[18][0]).src = IMAGE_PATH + 'source.gif';
		for (i = 0; i < TOOLBAR_ICON.length; i++) {
			var el = document.getElementById(TOOLBAR_ICON[i][0]);
...
function FtEditor(objName) 
{
	...
	this.skinPath = 'editor/skins/';
	this.iconPath = 'editor/icons/';
...
function CheckForm(form){
  ...
  title=title.replace(/(^\s*)|(\s*$)/g,"");  //去除标题左右两边的空格
  if (myform.BoardID_1.value==""){alert("请选择主分类！");myform.BoardID_1.focus();return false;}
  if (myform.BoardID_2.value==""){alert("请选择分类！");myform.BoardID_2.focus();return false;}
  if (myform.title.value==""){alert("请填写主题！");myform.title.focus();return false;}
  if (title.length<6){alert("文章标题最少要6个字！");myform.title.focus();return false;}
  if (title.length>45){alert("文章标题最长是45个字！");myform.title.focus();return false;}
  editor.data();//提交
  nl=document.myform.menu.value;
...
function CheckPollForm(form){
  ...
  if (myform.BoardID_1.value==""){alert("请选择主分类！");myform.BoardID_1.focus();return false;}
  if (myform.BoardID_2.value==""){alert("请选择分分类！");myform.BoardID_2.focus();return false;}
  if (myform.title.value==""){alert("请填写主题！");myform.title.focus();return false;}
  if (title.length<6){alert("文章标题最少要6个字！");myform.title.focus();return false;}
  if (title.length>45){alert("文章标题最长是45个字！");myform.title.focus();return false;}
...
function show_tb(pre,n,select_n)
{
	for(i=1;i<=n;i++)
	{
