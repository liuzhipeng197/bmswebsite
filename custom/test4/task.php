	...
	case"modify_req":
		//修改客户委托书
			...
			//计算预计费用和检验周期
				...
				$sql="select subject_name2 from subject where parents='$subject_id' and subject_name like '%报告份数%'";
                                $rep_num=get_result_first($sql);
	
				//分别获取用户申请的检验依据，检验项目，样品总数量对应的内容,此外还包括报告份数				
				 $sql="select $test_accord,$test_item,$sample_total,rep_num from $req_table where id='$req_id'";
				...
				if($rep_num_val!=''){
					if( $test_fee<=1000){
						$test_fee=$test_fee+200;//检测费用低于1000,报告加200
					}else{
						$test_fee=$test_fee+200*(int)$rep_num_val;//检测费用高于1000,每增加一份报告,需要加200
					}
				}
				$test_cycle=ceil($test_cycle/8); //将检验周期转成，以”天“为单位。
				$rep_num_val=(int)$rep_num_val;//转成整数
				//更新费用和时间和委托书号
				$sql = "update bms_task_reg set fee='$test_fee',cycle='$test_cycle',rep_num='$rep_num_val' where uid='$uid' and req_id='$req_id' and req_table='$req_table' ";
         	$result = mysql_query($sql,$conn);
         	if($result){
	...
	case"bus_del":					//删除业务申请
		...
		case"save_req_baseinfo":
			...
			if($result){
				...
				$sql="select $test_accord,$test_item,$sample_total,$rep_num from $table_name where id='$ins_id'";
				//echo $sql;
				$result=mysql_query($sql,$conn);
				$row=mysql_fetch_array($result);
				$test_accord_val=$row[0];
				...
				$req_date=date("Y-m-d");
				$req_time=date("H:i:s");
				$sql="insert into bms_task_reg(id,uid,req_id,sample_type,req_date,req_time,deal,req_table,fee,cycle,sample_total,rep_num) values('','$uid','$ins_id','$sample_type',
				'$req_date','$req_time','0','$table_name','$test_fee','$test_cycle','$sample_total_val','$rep_num_val')";
				$result = mysql_query($sql,$conn);
				if($result){
					echo "OK";
				}
