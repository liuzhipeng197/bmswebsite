 ...
 */
   ...
   /**
    ...
    {
        $filetime = @filemtime($compile_path);
        if (!$this->force_compile && $filetime > 0)
        {
        ...
        if ($this->_parse_resource_name($_params)) {
            ...
            switch ($_resource_type) {
                case 'file':
                    ...
                    if (!empty($_params['resource_timestamp']))
                    {
                        $params['resource_timestamp'] = $_params['resource_timestamp'];
                        $_return = $params['resource_timestamp'] > 0;
                    }
                    else
                    {
        ...
        if ($params['resource_type'] == 'file') {
            if (!preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', $params['resource_name'])) {
                ...
                foreach ((array)$params['resource_base_path'] as $_curr_path) {
                    ...
                    $filetime = @filemtime($_fullpath);
                    if ($filetime)
                    {
                        $params['resource_name'] = $_fullpath;
                        $params['resource_timestamp'] = $filetime;
                        return true;
                    }
                    else
                    {
                        $params['resource_timestamp'] = 0;
                    }
...
{
    if (strstr($params['results'], 'insert_cache'))
    {
...
function smarty_core_write_cache_file($params, &$smarty)
{
    ...
    if (strstr($params['results'], 'nocache:') && preg_match_all('!\{(/?)nocache\:[0-9a-f]{32}#\d+\}!', $params['results'], $match, PREG_PATTERN_ORDER)) {
...
function smarty_core_write_file($params, &$smarty)
{
    ...
    $size = strlen($params['contents']);

    $bytes = file_put_contents($_tmp_file, $params['contents']);
...
function smarty_modifier_escape($string, $esc_type = 'html', $char_set = 'ISO-8859-1')
{
    switch ($esc_type) {
        ...
        case 'hex':
            ...
            $return = '';
            for ($x=0, $count = strlen($string); $x < $count; $x++) {
