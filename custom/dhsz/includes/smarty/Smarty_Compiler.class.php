...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        for ($curr_tb = 0, $for_max = count($text_blocks); $curr_tb < $for_max; $curr_tb++) {
            ...
            if (strstr($text_blocks[$curr_tb], 'php') && preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $text_blocks[$curr_tb], $sp_match)) {
        ...
        if (count($this->_plugin_info)) {
            ...
            foreach ($this->_plugin_info as $plugin_type => $plugins) {
                foreach ($plugins as $plugin_name => $plugin_info) {
                    if ($plugin_name != 'insert_scripts' && $plugin_name != 'escape')
                    {
                        $info .= "array('$plugin_type', '$plugin_name', '" . strtr($plugin_info[0], array("'" => "\\'", "\\" => "\\\\")) . "', $plugin_info[1], ";
                        $info .= $plugin_info[2] ? 'true),' : 'false),';
                    }
                }
            }
            if ($info)
            {
                $_plugins_params = "array('plugins' => array(" . $info . '))';
                $plugins_code = "<?php smarty_core_load_plugins($_plugins_params, \$this); ?>\n";
    ...
    {
        /* Matched comment. */
        if ($template_tag{0} == '*' && substr($template_tag, -1) == '*')
            return '';
...
//                    return $output;
    ...
    function _compile_insert_tag($tag_args)
    {
        ...
        return "<?php echo smarty_core_run_insert_handler($_params, \$this); ?>" . $this->_additional_newline;
    }
            ...
            switch (strtolower($token)) {
                ...
                default:
                    if(preg_match('~^' . $this->_func_regexp . '$~', $token) ) {
                            // function call
                    } elseif(preg_match('~^' . $this->_var_regexp . '$~', $token) && (strpos('+-*/^%&|', substr($token, -1)) === false) && !empty($tokens[$i+1]) && $tokens[$i+1] == '(') {
